...
"""Trajectory reconstruction (time–velocity gating at 80 cm/s + synchronized
propagation + greedy concession + near-anchor damming)
轨迹重建（时间–速度门控 80 cm/s + 同步推进 + 贪心让步 + 近锚点堤坝止水）

- Use RFID anchors as "water sources" and spread only to non-anchor tracklets
- 以 RFID 锚点为“水源”，仅向【非锚点】tracklet 扩散
- Single-step time–velocity gating: ``d <= v_gate(px/frame) * gap`` and
  ``gap <= MAX_GAP_FRAMES``
- 单步“时间–速度门控”：``d <= v_gate(px/frame) * gap`` 且 ``gap <= MAX_GAP_FRAMES``
- Synchronized wavefront: all sources move one step per round
- 同步推进（wavefront）：一回合内所有水源各走一步
- Conflict resolution: minimum cost → ambiguity freeze (δ) → anti-hogging concession
- 冲突裁决：最小代价优先 -> 歧义冻结(δ) -> 反“抢食”让步
- Nearby anchors of the same tag within ``MAX_GAP_FRAMES`` act as dams and block expansion
- 同标签近邻锚点（MAX_GAP_FRAMES 内）视为“堤坝”，对应方向不扩散
- Write back ``chain_tag`` / ``chain_id`` / ``chain_origin``; supports repeated runs
...
PICKLE_OUT = None  # None=overwrite input; or provide new path None=覆盖输入；或给出新路径
OUT_SUBDIR = "CAP15"  # Subdir under input pickle dir; set None to skip 输出到输入pickle同目录下的子目录；设 None 则不用子目录

# Camera and gating
# 相机与门控
FPS = 30.0  # frames per second 帧/秒
PX_PER_CM = 14.0  # pixels per cm (camera calibration) 像素/厘米（相机标定）
...
# Synchronized propagation resolution parameters (for conflict resolution only)
# 同步推进裁决参数（只作用于冲突裁决，不改变门控本质）
EPS_GAP = 0.5  # cost = d + EPS_GAP * gap (0 disables)
DELTA_PX_CAP = 15.0  # ambiguity-freeze pixel cap (acceptable cost diff)
DELTA_PROP = 0.10  # ambiguity-freeze relative threshold (10% of gate)
...
# Re-run support
# 可重复运行支持
RESET_PREVIOUS = True  # clear old chain_* fields before running 运行前清理旧的 chain_* 字段
LOG_RUN_METADATA = True  # log parameters for this run 记录本次运行参数（安全写法）
...
def _head_center(
    tk_centers: Dict[int, Tuple[float, float]], start_f: int, n: int = HEAD_TAIL_SAMPLE
):
...
def _tail_center(
    tk_centers: Dict[int, Tuple[float, float]], end_f: int, n: int = HEAD_TAIL_SAMPLE
):
...
def _safe_center_from_arr(arr: np.ndarray, mc_idx, pcutoff: float):
    """Prefer ``utils.body_center_from_arr``; fallback to mean of keypoints with ``p>=pcutoff``
...
def summarize_tracklets(dd: Dict, pcutoff: float):
    """Extract per-tracklet information and build anchor indices
    提取每个 tracklet 的：start、end、centers、head、tail、tag、anchor_strength、is_anchor；
    并为每个 tag 建立锚点时间索引 ``tag_to_anchors[tag] = [(start, end, tk), ...]``。

    Robustness:
      - treat non-dict ``header`` as ``None``
      - process only dict nodes; skip others
      - frame key parsing: try ``frame_idx_from_key``, fallback to int
      - center computation: fallback to mean of keypoints with ``p>=pcutoff`` if utils fails
    健壮性：
...
# ================== Time–velocity gating & synchronized propagation ==================
# ================== 时间–速度门控与同步推进 ==================
...
def _delta_threshold_px(gap: int, v_gate_pxpf: float) -> float:
    # Ambiguity-freeze threshold δ: min(DELTA_PX_CAP, DELTA_PROP * gate limit)
...
def _collect_candidates(curr_tk, tk_info, assigned, direction: str, v_gate_pxpf: float):
    """
    Collect candidates of ``curr_tk`` in the given direction that pass the
    time–velocity gate:
      - only non-anchors and unclaimed tracklets
      - ``gap <= MAX_GAP_FRAMES`` and ``d <= v_gate_pxpf * gap``
    return ``[(cand_tk, d, gap, cost), ...]`` sorted by cost
    ...
    pool.sort(key=lambda x: x[3])  # sort by cost 升序
    return pool


def reconstruct_by_timespeed_gate(dd: Dict):
    """
    Main routine: rebuild chains via time–velocity gating + synchronized propagation + greedy concession
    ...
    while True:
        ...
        # Each source proposes its top-1 candidate (tracking whether alternatives exist)
        ...
        # 3) Conflict resolution (enhanced plan A)
        # 3) 冲突裁决（A 方案增强版）
        assignments = {}  # source_idx -> cand_tk
        used_sources = set()
        used_candidates = set()

        # Candidates processed in ascending order of best cost
        # 候选按最佳代价升序处理
        cand_order = []
        ...
        for _, cand_tk in cand_order:
            ...
            # Ambiguity freeze (difference of top two < δ)
            # 歧义冻结（前两名差 < δ）
            if len(lst) >= 2:
                second = lst[1]
            ...
            # Anti-hogging concession: if winner has alternative and a rival doesn't,
            # give it to the rival with minimal cost
        ...
        if not assignments:
            break  # no assignment this round, stop 本回合没有任何分配，终止

        # 4) Apply assignments: advance frontier, occupy candidate, write to chain
    ...
    # 5) Write back attributes and summarize CSV
    # 5) 写回属性并汇总 CSV
    chain_rows = []
...
def main():
    ...
    if OUT_SUBDIR:
        ...
        p_out = out_dir / p_in.name  # keep original filename 保留原文件名
    else:
        p_out = p_in if PICKLE_OUT is None else Path(PICKLE_OUT)

    if not p_in.exists():
        print(f"Error: input file does not exist: {p_in} 错误：输入文件不存在")
        return
    if str(p_in).endswith(".json"):
        print(
            f"Error: PICKLE_IN points to JSON not .pickle: {p_in} 错误：PICKLE_IN 指向 JSON 而非 .pickle"
        )
        return
    if FPS <= 0 or PX_PER_CM <= 0 or V_GATE_CMS <= 0:
        print(
            "Error: please set FPS, PX_PER_CM, V_GATE_CMS correctly (>0) 错误：请正确设置 FPS、PX_PER_CM、V_GATE_CMS（均需>0）"
        )
        return

    print(f"Loading pickle: {p_in} 正在加载 pickle: {p_in}")
    dd = load_tracklets_pickle(str(p_in))

    # —— Re-run: clear old chain fields ——
    # —— 可重复运行：清理旧链字段 ——
    if RESET_PREVIOUS:
        cleared = 0
        for tk, node in dd.items():
            ...
            if isinstance(node, dict):
                for k in (
                    ...
                    if k in node:
                        node.pop(k, None)
                        cleared += 1
        print(f"[reset] cleared old chain fields (≈{cleared} entries) 清理旧链字段完成")

    # —— Log run parameters (safe: do not modify non-dict header) ——
    ...
    print(
        "Starting trajectory reconstruction with time–velocity gating (80 cm/s) + synchronized propagation + greedy concession... 开始基于【时间–速度门控(80 cm/s) + 同步推进 + 贪心让步】的轨迹重建…"
    )
    dd2, df_chains = reconstruct_by_timespeed_gate(dd)

    # Save chain-segment CSV
    # 保存链段 CSV
    out_csv = p_out.parent / "chain_segments.csv"
    df_chains.to_csv(out_csv, index=False, encoding="utf-8")
    print(f"[OK] chain segments saved: {out_csv} 链段信息已保存")

    # Save updated pickle (overwrite or new)
    # 保存更新后的 pickle（覆盖或另存）
    save_pickle_safely(dd2, p_out)
    print(f"[OK] pickle file updated: {p_out} pickle 文件已更新")
    print(
        f"Reconstruction complete: generated {len(df_chains)} chain segments 重建完成：共生成 {len(df_chains)} 条链段记录"
    )
