...
"""RFID (reordered by coordinates → row-major ID) → tracklet hit counts
and conservative/intelligent tag assignment
RFID（按坐标重排→行优先 id）→ tracklet 累计读数 + 保守/智能 tag 分配

- Read ``readers_centers.txt`` (a column of 144 coordinates) and spatially
  rearrange into a 12×12 grid (top→bottom, left→right)
- 读取 ``readers_centers.txt``（一列 144 个坐标），按空间重排为 12×12 网格（上→下，左→右）
- Use row-major IDs (row 0: 0..11, row1: 12..23, …) to map coil centers
- 用行优先 id（第 0 行 0..11，第 1 行 12..23，…）映射到圈心坐标
- RFID events: map time to nearest frame; per-frame competition within radius
  with optional "unique nearest" filtering
- RFID 事件：时间→最近帧；逐帧在半径内竞争，支持“唯一最近邻”过滤
- Outputs: ``counts_by_track_tag.csv`` (hit counts), ``tag_assignments.csv``
  (assignment results), ``meta.json`` (parameters & diagnostics)
- 写出：``counts_by_track_tag.csv``（累计命中）、``tag_assignments.csv``（分配结果）、``meta.json``（参数与诊断）
- Write back to pickle: each tracklet node gains ``rfid_counts`` /
  ``rfid_frames`` / (optional) ``tag`` / ``rfid_hint``
- 写回 pickle：每个 tracklet 节点加入 ``rfid_counts`` / ``rfid_frames`` /
  （可选）``tag`` / ``rfid_hint``
"""
...
OUT_DIR = None  # None -> create rfid_match_outputs/ alongside pickle 与 pickle 同目录创建 rfid_match_outputs/

# ---- Array/grid settings (critical) ----
N_ROWS = 12  # number of rows 行数
N_COLS = 12  # number of columns 列数
ID_BASE = 0  # whether RFID CSV ids start at 0 (set 1 if starting at 1) RFID CSV 中的 id 是否从 0 开始（若从 1 开始则设为 1）
Y_TOP_TO_BOTTOM = True  # True: smaller y at top, larger at bottom; set False if opposite 画面上方 y 小、下方 y 大；若相反改 False

# ---- Geometry and matching thresholds ----
PCUTOFF = 0.35  # confidence threshold for mouse center / fallback centroid
RFID_FRAME_RANGE = 10  # frame window for competition (±range//2)
COIL_DIAMETER_PX = 170.0
HIT_MARGIN = 1.00
HIT_RADIUS_PX = (COIL_DIAMETER_PX / 2.0) * HIT_MARGIN  # hit radius in pixels 命中半径（像素）

UNIQUE_NEIGHBOR_ONLY = True  # require nearest neighbor to be unique 是否要求最近邻“足够唯一”
AMBIG_MARGIN_PX = (
    75.0  # difference < value → not unique (drop event) 最近与次近差距 < 此值 → 视为不唯一（丢弃事件）
)

LOW_FREQ_TAG_MIN_COUNT = (
    2  # ignore tags with global hits ≤ threshold 过滤：全局出现次数 ≤ 此阈值 的 tag 忽略
)
MIN_VALID_FRAMES_PER_TK = (
    1  # tracklet must have at least this many valid frames tracklet 至少有这么多有效帧才参与匹配
)

# ---- Tag assignment thresholds (conservative) ----
TAG_CONFIDENCE_THRESHOLD = 0.70  # dominant proportion ≥ 70% 主导占比 ≥ 70%
TAG_MIN_READS = 20  # total reads ≥ 20 总读数 ≥ 20
TAG_DOMINANT_RATIO = 3.0  # top1/top2 ≥ 3 第一名/第二名 ≥ 3
# Assign even if total_reads < TAG_MIN_READS when top proportion ≥ threshold 当总读数 < TAG_MIN_READS 且top占比≥该值时仍然指派
LOW_READS_HIGH_PURITY_ASSIGN = True
LOW_READS_PURITY_THRESHOLD = 0.90

# ---- Per-frame stability (optional) ----
USE_FRAME_STABILITY_CHECK = False  # enable per-frame stability check? 打开逐帧稳健性判据
BURST_GAP_FRAMES = 150  # gap ≥ value counts as new burst (~5s@30fps) 命中间隔≥此值计作新波段
MIN_BURSTS_IF_LOWHITS = (
    2  # require ≥2 bursts when total reads < 200 当总读数 < 200 时，要求 ≥2 个波段
)
...
def load_centers_txt(txt_path: str) -> np.ndarray:
    """Load reader center coordinates from text file
    从文本文件加载读卡器中心坐标
    """
...
def build_row_major_index(
    ...
    """Rearrange a column of ``N = n_rows * n_cols`` ``(x, y)`` positions into an
    ``n_rows × n_cols`` grid (top→bottom, left→right).
    将一列 ``N=n_rows*n_cols`` 个 ``(x,y)`` 位置按空间重排为 ``n_rows×n_cols`` 网格（上→下、左→右）。

    Returns
      - ``idx_map``: 1D array of length ``N`` where ``idx_map[row*n_cols + col]`` is the
        original line index (0-based)
      - ``centers_grid``: ``(n_rows, n_cols, 2)`` grid of ``(x, y)`` in row-major order
    返回：
    ...
    if not np.all(np.diff(row_means_y) > 0):
        print(
            "[WARN] row-mean y not strictly increasing; proceed with row-major order anyway"
        )
...
def parse_rfid_csv(csv_path: str) -> pd.DataFrame:
    """Parse an RFID CSV to standard columns
    解析 RFID CSV 为标准列
    """
    ...
    # Reader id (retain original base: 0 or 1)
    # 读取器 id（保留原始基准：0基或1基）
    id_list = []
...
def parse_timestamps_csv(ts_path: str) -> tuple[np.ndarray, np.ndarray]:
    """Parse timestamps CSV to arrays of frame indices and times
    解析 timestamps CSV，返回帧索引和时间数组
    """
...
# ---- Per-frame stability & assignment ----


def _count_bursts_from_frames(frames_list, gap_frames=150) -> int:
    """Estimate number of independent hit bursts from ``[(frame, conf), ...]``
...
def assign_tag_for_one_tracklet(
    ...
    """Conservative tag assignment with optional per-frame stability check.
    当 ``total_reads < TAG_MIN_READS`` 且 ``top_ratio ≥ LOW_READS_PURITY_THRESHOLD`` 时直接指派。
    """
    diag = {}
    if not counts_dict:
...
def main():
    ...
    # 1) Load and reorder coordinates into row-major grid
    #    加载并按坐标重排为行优先网格
    ...
    # Self-check: print corner coordinates
    # 自检：打印四角
    ...
    # Parse header to find mouse center index
    # 解析 header 找 mouse center 索引
    header = dd.get("header", None)
    mc_idx = find_mouse_center_index(header)

    # 3) Build mapping: frame -> [(tk, (cx, cy), mean_conf)]
    #    构建: frame -> [(tk, (cx,cy), mean_conf)]
    frame2points: dict[
    ...
    # Remove tracklets with too few valid frames
    # 剔除有效帧过少的 tracklet
    bad_tks = {
    ...
    # 4) RFID event preprocessing: map to nearest frame, bind coil center
    #    RFID 事件预处理：映射到最近帧，绑定圈心
    ...
    # 7) Output aggregated counts CSV
    #    输出累计读数 CSV
    rows_counts = []
    tag_names_all = set()
    ...
    # 8) Assemble meta.json (write parameters & diagnostics)
    #    组装 meta.json（先写基础参数与诊断）
    meta = {
        "params": {
            "pcutoff": PCUTOFF,
    ...
    # 9) Write back pickle with tag assignments
    #    写回 pickle + Tag 分配（一次完成）
    ...
    for tk, node in list(dd_out.items()):
        ...
        # Write back counts and per-frame data
        # 写回计数与逐帧
        ...
        # Assign tag (conservative + optional stability)
        # 分配 tag（保守 + 可选逐帧稳健性）
    ...
    # 11) Update meta.json with assignment stats
    #     更新 meta.json（加入分配统计）
    meta["tag_assignment"] = {
