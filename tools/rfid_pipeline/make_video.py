...
# ================== 绘制函数 ==================
def draw_tracklets_layer(
    frame, frame_idx, pts_in_frame, tk_rfid_events, tk_trail, tk_last_seen
):
    ...
    for tk, (x, y) in pts_in_frame:
        ...
        gap = frame_idx - tk_last_seen.get(tk, frame_idx)
        if gap > 1:
            tk_trail[tk].clear()
        tk_trail[tk].append((int(round(x)), int(round(y))))
        tk_last_seen[tk] = frame_idx
...
def main():
    ...
    while True:
        ...
        if fidx in valid_frames:
            ...
            canvas = draw_tracklets_layer(
                canvas, fidx, pts, tk_rfid_events, tk_trail, tk_last_seen
            )
